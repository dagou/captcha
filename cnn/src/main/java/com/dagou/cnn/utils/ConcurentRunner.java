...
public class ConcurentRunner {
	private static Logger logger = LoggerFactory
			.getLogger(ConcurentRunner.class);
	...
	/*
	 * public abstract static class Task implements Runnable { int start, end;
	 * public Task(int start, int end) { this.start = start; this.end = end;
	 * logger.info("new Task \t start {} end {}.", start, end); }
	 * 
	 * @Override public void run() { process(start, end); } public abstract void
